...
# -----------------------------
API_SECTION = "realtor_api"
API_KEY = st.secrets.get(API_SECTION, {}).get("key")
API_HOST = st.secrets.get(API_SECTION, {}).get("host", "realtor-search.p.rapidapi.com")
...
# -----------------------------
# API calls (Realtor Search)
# -----------------------------
def resolve_to_latlon(address: str):
    """ /properties/auto-complete?input=... -> (lat, lon, display) """
    try:
        url = f"https://{API_HOST}/properties/auto-complete"
        r = requests.get(url, headers=headers(), params={"input": address}, timeout=20)
        ...
        for h in hits:
            ...
            if lat and lon: return float(lat), float(lon), disp or address
        return None, None, None
    except:  # noqa
        return None, None, None

def fetch_nearby_values(lat, lon, radius_miles=2.0, limit=25):
    """ /properties/nearby-home-values -> comps list """
    try:
        ...
        for it in raw[:limit]:
            price = (
                it.get("price") or it.get("list_price") or it.get("value") or
                it.get("estimate") or it.get("avm", {}).get("value")
            )
...
def classify_risk(subject_price, subject_dom, comp_prices, comp_doms):
    ...
    if subject_price and c_price:
        ...
        reasons.append(f"Subject vs comps: {fmt_money(subject_price)} vs {fmt_money(c_price)} ({price_diff:+.1f}%).")
    else:
    ...
    if subject_dom is not None and c_dom:
        ...
        reasons.append(f"Subject DOM vs comps: {subject_dom} vs {int(c_dom)} ({dom_diff:+.0f}%).")
...
def build_pdf(report):
    ...
    pdf.cell(0, 8, "Subject (derived):", ln=True)
    pdf.set_font("Arial", "", 11)
...
st.caption("Type an address → we resolve it → pull nearby values/comps → compute risk & probability.")
...
if go and address.strip():
    with st.spinner("Resolving address and pulling comps…"):
        lat, lon, display_addr = resolve_to_latlon(address)
        if not lat or not lon:
            st.error("Could not resolve this address via Realtor auto-complete. Try another format.")
            st.stop()

        comps = fetch_nearby_values(lat, lon, radius_miles=2.0, limit=25)
    ...
    bar_chart_with_subject(subject_price, prices, labels, "Pricing vs Nearby Values (Subject proxy dashed)", "Price ($)")
